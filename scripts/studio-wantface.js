...
(function () {
  ...
  /* S2 polish: single-lever EXTRAS (changes-row + attribution delta) — VERBATIM slice of
   * sketch.html single-lever branch L5822-5838 + L5843-5867 (gated byte-identical against
   * scripts/_singlelever_baseline.fixture.js). Excludes the host-only ACCEPT/PTA _phys mods. */
  function _singleLeverExtras(ctx) {
  ...
  function _paintHUD(d) {
    ...
    if (nch === 0) {
      ...
      // S2 polish item 1: empty-state attribution — port Sketch's exact string (was a bare "—").
      if (lever) lever.textContent = 'No movement yet — adjust a slider to see lever attribution';
      ...
      // S2 polish item 2: changes-row (arrow transition) + attribution delta, via the shared
      // single-lever extras (Sketch parity). ds/ptsEnd mirror Sketch (s.targetSpend rounded).
      var _sx = _singleLeverExtras({
        wbCase: d.copy, gb: _haveScn,
        ...
        retire: _wantScn.activationAge, age: _wantScn.currentAge,
        ptsEnd: { datumSpend: Math.round(_wantScn.targetSpend) },
        gbPinnedState: { planThroughAge: _haveScn.planThroughAge || 93 }
      });
      if (lever) lever.textContent = _sx.domLever || '—';
      if (chRow) {
        if (_sx.changeHtml) { chRow.className = 'pin-changes-row'; chRow.style.display = ''; chRow.innerHTML = _sx.changeHtml; }
        else if (_sx.leverDelta) { chRow.style.display = ''; chRow.textContent = _sx.leverDelta; }
        else { chRow.style.display = 'none'; chRow.textContent = ''; }
      }
    } else {
