...
const SACRED = {
  ...
  'studio.html': 'de017d8ad1f0008612fc59f54d92b109',// ⛔ FINDING 61 — THE RESTORE BANNER HAD NEVER RENDERED, AND WHEN FORCED TO IT LANDED ON A LIVE CONTROL. showRestoredBanner() sat at top:60px; its only caller (restoreDraft) has never executed in the product's lifetime (F56), so nobody had ever seen it. Forced on localhost 2026-08-31 it overlapped the "Shape You Have" toggle by 116x27px at 12% ALPHA — both strings printed through each other, neither legible. 🔑 A COMPONENT THAT HAS NEVER RENDERED HAS NEVER BEEN LAID OUT EITHER: its copy was reviewable on paper, its GEOMETRY was not, and its conflicts were not absent but UNKNOWN. FIX: bottom-anchored ABOVE nav.js's #privacy-banner, whose 56px full-width fixed bar was MEASURED rather than assumed — a naive bottom:20px reproduces the defect at the other end, and the overlap is reachable (reload mid-session without clicking "Got it"). z-index 9998, deliberately BELOW the consent bar: if they ever meet the REGULATORY DISCLOSURE wins, not the convenience notice. ⛔ AND THE TIMED-ACTION DEFECT: the only route to the choice deleted itself after 9s. A NOTICE MAY EXPIRE, AN ACTION MAY NOT — reading the sentence cost the user the option it described. The notice now fades; the button stays. Copy wired VERBATIM (§82.934). clearDraftAndRefresh UNCHANGED — F61 is bounded to placement/legibility/timer/copy, and ← Overview is deferred as UNMEASURED. Gate: scripts/_gate_restore_banner_placement.js, 6 legs, red-first 3/6 (L1 L3 L4 RED · L0 L2 L5 GREEN as predicted). (2026-08-31)
