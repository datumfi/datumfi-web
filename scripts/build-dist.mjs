...
const SACRED = {
  ...
  'studio.html': 'b3aa0151ceb1080427ed408dac49a5a3',// ⛔ FINDING 52 / 53 — "START FRESH" DID NOT START FRESH. Banked as three hardcoded personal facts in clearDraftAndRefresh; MEASURED 2026-08-31 against a REAL COLD BOOT with all 29 controls driven, TWENTY-SIX FIELDS DIFFERED. Three were the fabrications (a birthday, a retirement date, and a spend that was not even the markup's own $100,000). The other TWENTY-THREE were THE DEPARTING USER'S OWN DATA surviving a button whose label promises to discard it — name, location, salary, tax rate, filing status, the co-architect's DOB and salary, every slider, every market/inflation/tax radio. The fix RE-DECLARES NOTHING: it restores each control to el.defaultValue / el.defaultChecked, the value the markup itself ships, so it cannot drift the way $120,000 drifted $20,000 from $100,000. It also clears data-exact-val, a SHADOW VALUE that outranks the slider position — leaving it behind made every field read cold while the HUD still showed the departing user's estate. Gate: scripts/_gate_start_fresh_cold_parity.js, 9 legs, 3 controls, expected state captured from a SECOND BROWSER CONTEXT rather than a written-down list. HANDLER-ONLY and says so: the banner button is unreachable until F56 wakes restoreDraft(). (2026-08-31)
