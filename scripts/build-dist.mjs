...
const SACRED = {
  ...
  'studio.html': '8dd278a3301aea293d919decd961bb72',   // ⛔⛔ A MISSING NUMBER WAS PRINTED AS $0K (2026-09-18). `fmtK` read `'$' + Math.round(n / 1000) + 'K'`, and Math.round(null / 1000) IS 0 — so a tier the engine DECLINED TO COMPUTE rendered as a confident "$0K/yr". Captain-found in a browser on his own ONE-YEAR plan (DOB 08/1982, retire 03/2045, plan through 03/2046): every SS-matrix cell read $0K/yr Total spending beside a $3,821K Lower spending estimate — a LOWER estimate ABOVE the total, which cannot happen from a healthy tier solve and was the tell that both figures were junk. 🔑 THE ENGINE WAS HONEST AND THE FORMATTER LAUNDERED IT: `null` on the wire says "I did not compute this"; `$0K` says "I computed this, and it is nothing". A NULL RENDERED AS A NUMBER IS WORSE THAN A CRASH — a crash tells the user to come back later; this tells them their retirement is worth nothing. ⚠️ THE EM-DASH IS THIS SURFACE'S EXISTING NO-ANSWER GLYPH (.ss-cell-empty already draws one), so no new vocabulary is invented, and it is deliberately NOT a zero, NOT a blank and NOT the nearest value. ⛔ `_fmtK` IN THE INSPECT PANEL WAS A SECOND COPY CARRYING THE SAME DEFECT and now DELEGATES rather than being patched twice — same script block, scope verified not assumed. ⚠⚠ TWO ENGINE DEFECTS REMAIN OPEN AND THIS DOES NOT FIX THEM, IT ONLY STOPS THE CLIENT LAUNDERING THEM: (1) the SS matrix SUCCEEDS on a household /api/calculate REFUSES with 422 range_not_drawable, and (2) the tier solve degenerates at short horizons (keystone null, bedrock 3,821,000). MEASURED: the ladder is sane and monotonic from a 3-year horizon up and only 1- and 2-year plans refuse. A DISPLAY GUARD IS NOT A FIX FOR THE THING IT IS GUARDING AGAINST.   // ⛔⛔ NINE EMPTY CELLS   // ⛔⛔ NINE EMPTY CELLS FOR EVERY SOLO HOUSEHOLD, AND IT WAS ALREADY LIVE (2026-09-18). The SS timing matrix rendered a 3x3 'Secondary / Spousal Claims At' grid unconditionally and keyed every cell `p + '_x_' + s`. A SOLO run is answered by the engine on ONE axis - case_key 'age_62', never 'age_62_x_None', because naming a pairing that does not exist would be a lie - so all nine lookups missed and all nine cells drew an em-dash. MEASURED against the live engine: solo returns 3 cases keyed age_62/age_67/age_70, joint returns 9 keyed age_NN_x_age_NN. ⛔ CAPTAIN-FOUND IN A BROWSER, and it shipped in da43fa6 which has been on origin/main for days - every solo household that ever opened this surface saw nine dashes. ⛔ CLAUSE 1 IN ONE SCREEN: joint worked, solo did not; the product asked a person with no spouse which age their spouse would claim at, then answered nothing. ⭐ A SHAPE FIX ONLY - the solo copy was ALREADY AUTHORED in the tradeoff chain's final else branch and had never once rendered, and the cell body is SHARED by both shapes rather than forked, so they cannot drift. 🔑 SOLO IS DERIVED FROM THE ENGINE'S OWN ANSWER (no '_x_' in any case_key), NEVER from the co-architect toggle: a control can disagree with what was actually computed, and a renderer that trusts a control over its own data can draw a grid for a household the engine never modelled. ⚠️ renderMatrixGrid is now an EXPLICIT window seam - I assumed it was already a global because this inline block 'is not an IIFE', the gate's L0 read fn:false, and the assumption was simply wrong.   // ⛔⛔ THE RETIREMENT AGE WAS DERIVED TWO WAYS AND THEY DISAGREED FOR 29.2% OF HOUSEHOLDS (2026-09-18). The typed-date VALIDATOR asked date-bounds.js and got a MONTH-AWARE age; the PAYLOAD read `.split('/')[1]` and threw the month away. DOB 08/1982 + retirement 03/2045 is 62 to the validator and 63 to the request. MEASURED: 42 of 144 DOB-month x retirement-month pairs disagree. ⛔ IT SHIPPED A ZERO-YEAR PLAN — Captain-found in a browser: the validator accepted a plan-through as "one year after retirement", the payload then sent a retirement age one year higher, and the engine received plan_end_age == retirement_age, indexed an empty array, and the household read "Map computation failed / index 0 is out of bounds for axis 0 with size 0". ⚠️ AND AWAY FROM THAT BOUNDARY IT NEVER CRASHED — IT FLATTERED: Keystone +$3,000 / +$5,000 / +$4,000 / +$11,000 a year across four households at a normal plan-through of 93, ALWAYS UPWARD, because a later retirement is one more year of accumulation and one fewer of drawdown. That is why FIVE sites carried it with every gate green. 🔑 ONE DERIVATION NOW, DELEGATED TO date-bounds.js — `ageOnDate` is an adapter, not a sixth spelling. Four sites repaired; the fifth (`_validateDateField`) receives an AGE and never the DOB string, so it is NAMED IN PLACE rather than silently left. ⛔ SECOND RULE IN THE SAME COMMIT: the plan-through floor moves from RA to floor(RA)+1 in BOTH date-bounds.js and `minPlanEnd`, matching the same-day repair to `schemas.validate_ages` — the standing order written above minPlanEnd is that the two floors are ONE RULE IN TWO REPOSITORIES AND MOVE TOGETHER OR NOT AT ALL. Plan-through == retirement age is the "ZERO DECUMULATION YEARS" case the Captain's own 09-13 ruling was written to forbid, and both files were still accepting it. ⚠️ BOTH GATES OVER THIS RULE WERE COMPLICIT: `_gate_plan_window_never_empty` B2 ASSERTED floor == RA exhaustively over 46 ages, and B7c RE-TYPED studio's expression by hand — a third copy inside the gate written to prove there were not two. `_gate_plan_through_typed` was worse: its fixture used the SAME MONTH for DOB and retirement (one of the 102 of 144 pairs that agree), so it was structurally incapable of seeing the defect it stood over, and its L3 still demanded the `RA+20` floor struck on 09-13. All three repaired; L6 is new and red-first proven.   // ⛔⛔ THE SHARE-DELIVERED SWAP (2026-09-18). Six labels retired the forbidden word, which survived an entire arc because it lived in LABELS and nobody re-reads a label. 'Plan confidence' -> 'Plan delivery'; 'Confidence at this spend' -> 'Delivered at this spend'; the chart title, the section label and the foot copy likewise. All five strings are the Architect's, verbatim, from 'Copy Block' Part 4. ⛔ THE FAILURE RATE TILE IS RETIRED, NOT RENAMED: it was 100 minus a confidence figure — ARITHMETIC, NOT AN OBSERVATION — and under share-delivered its complement is the delivery number said backwards. The whole-rate is what belongs in that corner and IT IS NOT WIRED YET: the engine returns it, the four rung companions are written, THE TILE LABEL IS NOT. Raised by name; the slot stays empty. ⚠️ THIS CLIENT REQUIRES AN ENGINE CARRYING share_delivered — :prod-16 does NOT, verified against the live endpoint. These commits cannot ship without a new image, and there is deliberately NO FALLBACK to the old field: the panel shows its empty state rather than printing the wrong measurement under the right label.   // ⛔⛔ THE MARKET CLIMATE LEFT THIS FILE (2026-09-18). Four tiles became five Model Designs in scripts/studio-market-climate.js, their CSS in styles/market-climate.css, and the shell keeps an empty .climate-grid — its ABSENCE is visible, it can never be WRONG. 🔑 DELETION COUNTED AS PORTING: the four weighting tables came out rather than being corrected, because all four stated model weights the engine does not use and the DEFAULT one was INVERTED against its own copy (CAPE shown at 15%, the lowest of its four, beside 'Trusts valuation-aware models more', while the engine weighted CAPE at 40%, the highest). A WRONG NUMBER SHOWN TO A HOUSEHOLD IS WORSE THAN NO NUMBER BECAUSE IT IS A NUMBER THEY CAN ACT ON. ⛔ AND THE THREE LABEL->ENUM MAPS ARE DELETED, NOT SYNCHRONISED — two lived here and one in studio-blueprint.js, and A RULE KEPT IN THREE LANGUAGES HAS ALREADY DRIFTED. data-outlook-key now carries the engine's own market_outlook enum, so the DOM holds the answer and there is nothing left to translate; the only surviving translation is the four retired aliases, in ONE home. ⚠️ THE <script src> IS IN-BODY AND POSITIONED, NOT PLACED: it runs at parse time so the tiles exist before three existing consumers that do not guard for their absence — the readiness check, studio-blueprint's commit listener, and the payload read. defer or <head> renders tiles with nothing listening, silently. ⚠️ The superseded intro ('calibrates the algorithmic blend') went with it: true of four presets, FALSE of five designs, and nearly unreadable in light mode — caught by a SCREENSHOT, not by a gate.   // ⛔ THE SALARY-BLANK MATCH CASE IS REPORTED (2026-09-17 late). A match set as a PERCENTAGE OF PAY with no pay to apply it to could never be SIZED, so unlike the vesting-blank case there is no dollar figure to withhold — and without a flag the engine sees an ordinary contribution and has nothing to be suspicious about. Copy Bank row 308, N-MATCH-NO-SALARY. Third distinct match condition: partially vested is a number we HAVE and are honouring; vesting blank is a fact we LACK and are withholding money over; salary blank is a match we cannot compute at all.   // ⛔⛔ THE 95% ALLOCATION CLIFF IS GONE (2026-09-17). `_householdAssetMix` returned NULL when less than 95% of portfolio value was classified, and NOBODY EVER RULED THAT THRESHOLD. Returning null means the engine falls back to its own default, so A HOUSEHOLD WHO CLASSIFIED 94% OF THEIR MONEY - who did almost all of the work - WAS HANDED A STRANGER'S ALLOCATION FOR ALL OF IT, INCLUDING THE 94% WE COULD SEE. Silent, and failing in the worst available direction. 🔑 THE CLIFF IS NOW A SLOPE: this function ships the FACTS - the mix of the classified slice and what share of the portfolio that slice is (`allocation_classified_pct`) - and the ENGINE blends its neutral default into the remainder by value and discloses the unclassified dollars by name. Which default fills the rest is a RULE, and a rule kept in two languages drifts (L48). MEASURED: nothing-classified keystone 126,000 against 94%-classified 137,000 - 11,000/yr apart where the two used to be identical. ⚠️ The all-or-nothing form survives for the one case it is correct in: NOTHING classified. ⛔ ALSO: `match_excluded_annual` now travels - the employer match the client could not count because vesting is blank. The engine cannot know a match exists and was excluded; it would see a smaller contribution and report nothing, which is a conservative assumption with no disclosure and therefore indistinguishable from a defect (§82.2702). Verified on a stub: primary with vesting stated sends 33,000 of contribution, the co-architect with vesting blank sends 6,000 and 4,000 of excluded match beside it.   // ⛔⛔ THE EMPLOYER MATCH REACHES THE ENGINE (2026-09-17). This file has carried a whole §8 401(k) MATCH ENGINE all along - `_di401kMatch` reads matchRate, matchUpTo, vestedPct and the household's own Gross-Salary and produces an annual match IN DOLLARS - and the payload sent `inflow x freq`, WHICH IS THE EMPLOYEE DEFERRAL ONLY. Every dollar the employer put in was invisible to the engine for the whole accumulation. MEASURED: an employee deferring 24,000 with a 100%-up-to-6% match on a 150,000 salary is 9,000/yr short - 66.7000% to 44.5250% confidence and 257,914 of portfolio at retirement. IT UNDERSTATES, which is why nothing was pointed at it. ⭐ FOUND BY `npm run census:controls` ON ITS FIRST RUN, by nobody suspecting it - the instrument for §82.2690's fourth failure mode, built the same day. 🔑 REUSED, NOT REBUILT (L48): `_di401kMatch` is CALLED, so the number the engine is told is the same number the modal shows. A second copy of the formula would agree on the day it was written and drift the first time the match rules moved. ⛔ AT THE VESTED PERCENTAGE, NOT THE GROSS (§82.2704) - unvested employer money is money the household does not own yet and may never own. ⚠️ WHERE VESTING IS BLANK, NOTHING IS COUNTED, which is deliberately identical to today's behaviour so this line cannot move a number for a household that has told us nothing - but it is UNDISCLOSED, §82.2702 forbids that, and the string is the Architect's to author. FLAGGED, NOT INVENTED. ⚠️ Salary is the Profile's own field per room owner (co-salary for a _co room, pri-salary otherwise), read exactly as §8's own caller reads it - Captain-ruled §82.2705, the field was collected and had no consumer.   // ⛔⛔ THE RULE OF 55 REACHES THE ENGINE (2026-09-17). `rule_of_55_eligible` APPEARED NOWHERE IN THIS FILE. The Studio has a `useRule55` toggle, and switching it on prints an authored paragraph - “You’ve switched on Rule of 55 ... this account opens four years early, penalty-free” - while the payload never carried the flag and the engine’s field defaulted to false. So trad_401k_r55_balance WAS ZERO FOR EVERY HOUSEHOLD THAT HAS EVER USED THE STUDIO, and yesterday’s per-owner repair to that mechanism (including the solo case, structurally impossible before) was a repair to a pool NOTHING EVER FILLED. MEASURED, one household retiring at 56 on a 1.4M 401(k): 39,994 dollars of lifetime early-withdrawal penalty and 1.23 points of confidence. THE PRODUCT CHARGES A PENALTY IT TOLD THEM IN WRITING THEY WOULD NOT OWE. §82.2673, third field on this one line after owner and cost basis. ✅ NULL RESULT, RECORDED SO NOBODY RE-CHASES IT: `exclude` is written false at creation and NEVER SET TRUE anywhere - an inert Phase-III hook, not a live defect.   // ⛔⛔ THE HOLDINGS LAYER REACHES THE ENGINE, AND THE CONTRIBUTION UNIT WITH IT (2026-09-17). ONE LINE built the account payload and it reduced every room to four keys - type, balance, annual_contribution, owner - discarding TWO things that change money on screen. (1) COST BASIS AND THE WHOLE FIFTEEN-FIELD HOLDINGS LAYER: seeded on every row, typed and stripped on input, driving a per-holding embedded gain and an account-level Unrealized Gain, with a reverse-growth ESTIMATOR and a 'Finding your cost basis' coaching panel behind it - and a shipping tooltip promising 'We use it to find your real embedded gain, so the capital-gains tax is exact instead of a guess.' MEASURED in the engine: a taxable brokerage and a cash account priced IDENTICALLY to a Roth, 98.2250% / $2,868,731.42 to the cent, while that sentence shipped. ⛔ (2) CONTRIBUTIONS ARE PER PERIOD AND WERE SENT AS PER YEAR. The box is a rate with a unit beside it - / YR, / MO, / WK, / DY - DEFAULTING TO / MO, and ELEVEN places in studio.html annualise it as inflow x freq. This line was the twelfth and sent inflow alone. MEASURED, a household 20 years out contributing $37,200/yr: 99.2500% -> 76.4750% confidence and $977,207 less portfolio at retirement. ⭐ IT UNDERSTATES, WHICH IS WHY SIX AUDITS HUNTING FOR FLATTERY ALL MISSED IT. ⛔ §82.2672 - A LINE ALREADY CAUGHT DISCARDING ONE THING MUST BE READ FOR EVERYTHING ELSE IT DISCARDS BEFORE IT IS CLOSED: this is the same line that dropped `owner`, and nobody asked what else it dropped. It dropped fifteen more. ⚠️ RATES ARE PERCENTS HERE AND DECIMALS ON THE WIRE - dividendYield and intRate are both scaled at this boundary, and the engine refuses anything above 1.0 at the door so the conversion cannot go missing quietly. ⛔ HELD WITH THE OTHER TWO: the engine must land in the same window or holdings are dropped by pydantic and the freq repair ships alone.   // ⛔⛔ WHOSE ACCOUNT THIS IS REACHES THE ENGINE (2026-09-17). ACCOUNT_TYPE_MAP holds 36 baseIds — 15 with a _co suffix, 5 with _primary — and the payload builder collapsed every one onto the same owner-blind enum: pretax401k and pretax401k_co arrived as the identical word. The Studio has modelled fifteen co-architect rooms the whole time and discarded the person on one line. MEASURED in the engine 2026-09-16: $2.0M in one person's 401(k) and $1.0M each returned the BYTE-IDENTICAL ladder, because every age-gated rule — the 10% early-withdrawal penalty above all — was applied to a merged fiction with one birthday. ⭐ DERIVED FROM THE MAP'S OWN KEYS (/_co$/), never a hand-written list of fifteen ids that the 36th baseId would not join. ⛔ HELD, NOT DEPLOYED: it makes account_owner_has_no_person reachable — a _co room can outlive the co-architect toggle — and that refusal string is [COPY-OWED].   // ⛔⛔ THE TIMING PATH THE USER CHOOSES IS THE ONE THE ENGINE IS SENT (2026-09-16). `lockTimingPath` moved the gold styling, moved the "Your Plan" badge, wrote a sessionStorage key and printed "Timing path set." — AND TOUCHED NONE OF THE CONTROLS THE PAYLOAD READS. `buildStudioRequest` takes the primary claiming age from `.ss-btn.active` and the secondary from `.ss-sec-btn.active`; `ss_timing_locked_key` had three readers and NOT ONE OF THEM WAS THE REQUEST. So the product marked a cell THIS IS YOUR PLAN, confirmed it in writing, ticked a readiness box, and computed a different plan — Captain-found in a browser, clicking the top-left cell. 🔑 A CONFIRMATION IS A PROMISE THAT THE SYSTEM ACTED, and THE STRENGTH OF THE CONFIRMATION IS WHAT MADE IT A LIE: a silent click would merely have failed; this one was believed three times over. ⚠️ REAL MONEY, MEASURED IN THE ENGINE ONE VARIABLE AT A TIME: keystone 112,750 at full_67, 108,750 at early_62 (-4,000), 115,250 at optimal_70 (+2,500). ⛔ ALL THREE SURFACES MOVED IN ONE COMMIT: the click now writes both button groups; the badge key is DERIVED from those controls so it cannot name a plan the engine will not be told; and the SS Timing readiness tick asserts the live controls instead of a key no request consults. ⚠️ A SOLO KEY WRITES NO SECONDARY — inventing one would re-manufacture a claiming age for a co-architect who does not exist, the defect `_current_plan_key` was repaired for on both sides of the wire. scripts/_gate_timing_path_reaches_payload.mjs runs the REAL function against a stub DOM and reads the buttons back; RED-FIRST against the pre-repair file it reports `primary button = 67` — the exact symptom, not a crash.   // ⛔⛔ THE HOUSEHOLD'S OWN ASSET MIX REACHES THE ENGINE (2026-09-16). `_householdAssetMix()` sums equity / bond / cash DOLLARS across every engine-bound room through the Studio's own `_diSignals` classifier and sends three fractions beside `location` and `filing_status`. THE CLIENT HAS CLASSIFIED HOLDINGS SINCE LONG BEFORE THE ENGINE COULD RECEIVE ONE -- it prints "The invested sleeve is NN% stocks" in six places and drives Roth asset-location advice off bondPct>=30 -- AND THREW THE NUMBER AWAY AT THE PAYLOAD BOUNDARY, while the engine modelled 80/17/3 for every household on earth. MEASURED on the Captain's own 15-Sep save through that same classifier: 94.6% stocks / 4.7% bonds, 100% of invested value classified. Worth up to $14,000/yr of Ceiling (household A) and $13,000/yr (household B) against the mix he was actually being modelled at. ⚠️ THE DENOMINATOR IS THE ENGINE'S PORTFOLIO, NOT THE SUM OF THE HOLDINGS: `_diSignals` only sees rows in the holdings table, so $900k typed straight into a 401(k) beside $10k of classified stock would otherwise report 100% EQUITY from a 1.1% sample. Below 95% coverage it returns null and SENDS NOTHING, and the engine states its own assumption in `allocation.source` where a reader can see it. ⭐ The SS matrix is covered by the same expression -- buildMatrixRequest builds on this body (Finding 67's repair) -- so both egresses describe one estate.   // ⛔⛔⛔ THE CUTOVER — THE STUDIO DRAWS ITS OWN RANGE (2026-09-14). Reveal no longer walks the user to range.html; it forces Split (§82.2354), hands the fetch promise to the Convergence swarm and opens #mcOverlay in place. THE PANEL AND THE SWARM WERE BUILT AND UNREACHABLE THE WHOLE TIME — grep DatumMeasurement studio.html returned ZERO LINES while :prod-8 was returning tiers, a 43-point capacity_curve and the tax series on every reveal, all discarded. ⛔ FOUR OTHER REPAIRS RIDE IN THE SAME COMMIT AND EACH ONE WAS CAPTAIN-FOUND IN A BROWSER: (1) the plan-through door now tests window._ptaAnswered instead of a box the product filled in — typing DOB 08/1982 in the Profile was writing 08/2072 into #plan-end-age and closing the door with the slider's shipped value="90" (§82.2350, §9.3); the Sketch path was already correct and did not move. (2) Every surviving "|| 93" and "{value: 93}" deleted, including one inside a live expression (defaultPlanEnd) whose only consumer was _planEndAgeDefault, a variable NOTHING EVER READ — the Captain's 93 sitting on the plan-through window, invisible to any "what moves the answer" sweep because it moved nothing, and one wiring commit away from shipping as a recommendation. A DEAD READER IS NOT A SAFE DEFAULT; IT IS A DEFAULT WAITING FOR A CONSUMER. (3) The SS matrix key translation: datum-fi cbc231f canonicalised the claiming strategies to age_NN and kept the old names as INPUT aliases only, so the engine emitted age_67_x_age_67 while this file asked for full_67_x_full_67 — MEASURED in production, 0 OF 9 KEYS MATCHED and the grid drew nine em-dashes over nine correct answers (§82.2349). (4) The tenth reveal door deleted: it demanded an account with a BALANCE, the rule the Captain overturned, and it was STRICTER than the estate door it duplicated — a new 401(k) with a zero balance and a monthly contribution cleared door 5 and was stopped by door 10 (§82.2351). ⚠️ #cinematic-overlay is now dead surface, deliberately uncalled rather than deleted — resetOverlayState() is still the one home of the button re-enable. ⚠️ window._datumWasRefusal is READ for the first time: a refused household (New Jersey today) used to have its refusal painted and then be navigated away from it.
  ...
  'scripts/studio-measurement.js': '2cb0703a375ce7cfab0d6055a351d9be',   // ⛔⛔ THE PANEL READS share_delivered (2026-09-18). Relative guardrails make a household SPEND LESS rather than fail, so the success rate saturates at 1.0000 across a wide band and stops discriminating — measured on one household across the full grid, success_rates runs 1.0000 -> 0.0000 as a STEP while share_delivered runs 1.000000 -> 0.221790 as a CURVE. 🔑 The old number is not merely coarse, it answers a different question: 'how often did I avoid disaster' is not 'how much of the life I planned did I actually get', and the second is what a Range is for. ⛔ mcFailure is retired with its tile and removed from DATA_SLOTS — a slot left in that list writes to an element that no longer exists, which is a silent no-op that outlives everyone who remembers why.   // THE MEASUREMENT ROOM. Pinned in the commit the NUMBERS ARRIVED, which is the trigger its own header set when it shipped inert: 'PIN IT THE COMMIT THE NUMBERS ARRIVE. A deferral with no trigger is a gap.' It now owns the ONLY mapping from the engine's capacity_curve onto this panel's axis — and the engine's spend grid is 4-6x WIDER than the window (measured: grid 18,225-248,225 against a window of 31,000-77,000), so a wrong resample draws a confidence collapse across the user's own range with the axis labels still correct and nothing thrown. Its absence is BLANK, never wrong; its CORRUPTION is wrong and silent. That asymmetry is the pin's whole job.
  ...
  'Blueprint.html': 'b89ff38c716a5a95f249b73ce3b308f4',   // (0c) links styles/tokens.css; page-local shadows of shared names removed (2026-08-16)
