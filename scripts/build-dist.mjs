...
const SACRED = {
  ...
  'studio.html': '451b9c595d02b5efe527020c80840446',// ⛔⛔ F64 — THE PROFILE NOW CAPTURES WHAT THE USER TYPES. MEASURED, all 13 #sec-profile controls typed -> reload -> the preserving X: 5 of 13 came back BEFORE, 11 of 13 AFTER (the two salaries have NO schema slot and travel separately, DECLARED). ⛔ F64 WAS FILED AS A RESTORE DEFECT AND IS A CAPTURE DEFECT — restoreDraft returned all six keys the draft actually held; eight fields never reached it. The filed 'cheapest win, no schema work' (read co_architect_dob/co_architect_retirement_date back out) was a NO-OP: both are '' in a real draft, written only by applyDossier from the Dossier. ⚠ co-dob IS NOT COSMETIC — buildMatrixRequest (:17294) derives the co-architect's age from it for the SS Matrix payload, so a joint household that reloaded was planned as a SINGLE one, silently. ⛔ plan-end-age is restored HERE and not left to seedFromBlueprint, and my first read was wrong: that receiver exists and is correct but runs BEFORE #pri-dob is set, and _mirrorPlanEnd correctly blanks the field with no sourced DOB (F51) — A RECEIVER THAT RUNS BEFORE ITS PRECONDITION IS NOT A RECEIVER. ⛔⛔ THE TAX SELECTS FAIL TOWARD BLANK: the schema ships FL / Married Filing Jointly / 0.20 and an unguarded restore writes Florida into a select the user never touched, so a stored value counts as an answer ONLY if it differs from the LIVE schema default (never a literal copied here — that would fail OPEN the day the schema moved). MEASURED with the guard removed: filing-status came back 'Married Filing Jointly' for a user who chose nothing. It can under-restore; it can never fabricate. ⚠ pri-location IS CAPTURED AND IS INERT — studio.html:14228 sends location:'FL' as a LITERAL (F65), so this changes nothing the engine computes; it ships DECLARED, not implied fixed. Gate: _gate_profile_population_roundtrip.js, 5 legs, roster ENUMERATED FROM THE DOM (never the number 13). RED-FIRST on pre-fix bytes: 3/5, L1 naming all six. (2026-09-01)
