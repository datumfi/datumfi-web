...
const SACRED = {
  ...
  'studio.html': 'bf5fda5baf6ab25b75814daaee6ba0d0',// ⛔ FINDING 58 — A DEFERRED ACCEPTANCE WAS BANKED WHERE AN APPROVAL LIVES, AND THE FIX ONE COMMIT EARLIER IS WHAT CREATED THE PATH. _enforceProfileDate banks el._lastGoodDate whenever res.ok; F54's unanchored acceptance IS res.ok, so a value NOBODY HAD JUDGED was recorded in the slot reserved for values judged GOOD, and _rejudgeDependents named the conflict without ever retracting that snapshot. CAPTAIN-FOUND ON PRODUCTION LESS THAN AN HOUR AFTER F54 SHIPPED, by typing a SECOND bad date that no script asked for: co-dob 03/2000 + retirement 03/2030 correctly warned "puts them at 30", then typing 03/2040 REVERTED THE FIELD TO 03/2030 — the very value the product had just called impossible — while the authored sentence explaining why was replaced by the generic bound message. THE VALUE CAME BACK AND ITS EXPLANATION WENT AWAY. Reproduced in the harness on BOTH persons before a line changed. ⭐ FIX IS ONE LINE, AND ITS PLACEMENT IS THE WHOLE ARGUMENT: `delete el._lastGoodDate` in the CONFLICT branch only — THE SNAPSHOT DIES WHEN IT IS DISPROVEN, NOT WHEN IT IS MERELY UNJUDGED. The broader fix (never bank a deferred value at all) would have passed every F58 leg and quietly AMPUTATED the ordinary unanchored revert-to-last-good courtesy, where we have no grounds to call the value bad — the honest-half law applied to a STATE rather than a feature. ⛔ NEW LEGS L10 + L11/L11b: L10 asserts a second invalid edit cannot restore the disproven value; L11 is a BICONDITIONAL — the conflicting value and its authored explanation are present or absent TOGETHER — so it also reds the INVERSE fix that clears the field but leaves the stale sentence standing (C7 proves that direction is live). ⭐ L11 IS GREEN IN BOTH STATES at the moment of conflict, which is what lets L11b isolate the failure to the SECOND edit rather than to the conflict itself. Controls: C6 restores the F58 defect verbatim (L10 + L11b, both persons); C7 is a strict subset (L11b only). 🔑 THE GATE GAP THAT ALLOWED IT: L4/L5 stop at the moment the conflict is NAMED. A LEG THAT PROVES A STATE IS REACHED MUST BE FOLLOWED BY ONE THAT PROVES THE STATE IS SURVIVABLE — stopping at the moment of success is how a gate certifies a snapshot and misses a sequence (2026-08-30) // ⛔⛔ FINDING 54 — THE CO-ARCHITECT WAS BEING JUDGED AS THE PRIMARY. _profileDobAnchor, the anchor for every Profile target-date validation, MANUFACTURED its anchor when no DOB was supplied: { mo: 1, yr: thisYear - PRIMARY slider-age, age: slider-age }. _enforceProfileDate then REVERTED-OR-CLEARED the user's typed date on that basis — so a fabricated fact did not merely mislead, IT ERASED USER INPUT. MEASURED IN FOUR ARMS BEFORE A LINE CHANGED, with two singleton pairs isolating the cause: (1) co-ret "06 / 2030" with no co-dob was REJECTED AND ERASED at primary slider-age 40 and ACCEPTED at 60 — identical co-architect state, identical typed date, only the OTHER PERSON's slider differed. A fabricated value borrowed from another person does not just invent: IT ASSERTS A RELATIONSHIP NOBODY STATED AND THEN ENFORCES IT AGAINST ONE OF THEM. (2) co-ret "06 / 2031" was ACCEPTED with co-dob EMPTY and REJECTED AND ERASED once the co-architect's TRUE December birth month was given — THE MANUFACTURED JANUARY WAS STRICTLY MORE PERMISSIVE THAN REALITY, so the product PUNISHED THE USER FOR ANSWERING HONESTLY. Neither of us predicted that arm; it came from a control run anyway. ⭐ FIX, Architect-ruled: a validator's job is to reject the IMPOSSIBLE, and with no anchor there is no impossible value — so _profileDobAnchor returns NULL and a well-formed date is accepted IN SILENCE (no "enter a date of birth first" message exists, deliberately). ⛔ DEFERRED IS NOT ABANDONED: _rejudgeDependents judges the deferred date the moment its DOB lands, READING _PROFILE_DATE_META's `anchor:` BACKWARDS rather than re-declaring which field anchors which — a second list is a second opinion and drifts the day a field moves. ⛔ IT NAMES THE CONFLICT AND ERASES NEITHER VALUE (the user supplied both facts; the product may not choose which to discard), and {age} comes from validateTarget's OWN return so the sentence and the decision cannot disagree. ⚠️ P8.1's revert-or-clear SURVIVES for the field in hand: a product may correct the field the user is looking at, not one they are not — the cost of a correction is not the value destroyed but the chance it goes unnoticed. ⚠️ ORDERING IS LOAD-BEARING: _updatePlanEndAge ALSO owns ret-date-warn and hides it, so the re-judge MUST run last or its message is silently erased. ⭐ ONE CAUSE, FOUR SURFACES — splitting would fork a shared validator four ways (L48). Guarded by scripts/_gate_profile_anchor_deferred.js: 11 legs driving the real UI, predicted red-first AND green-first and both confirmed (L1 L2 L3 L4a L4b L5a L5b L9 red, L6 L7 L8 GREEN ON THE UNFIXED BUILD — a leg green both times guards the honest half), five controls with disjoint red sets including C5, the AMPUTATION control, which proves "just stop validating" cannot pass. ⛔ STILL LIVE, FILED AS F57: _readDobMoYr carries the identical manufactured anchor for plan-end-age — six callers, a different cause, NOT fixed here (2026-08-30) // ⛔⛔ FINDING 49a — THE LAST TWO INVENTED PERSONAL FACTS, BOTH MEASURED BEFORE BEING CLAIMED. (1) _applySketch carried a saved Sketch into the Studio and wrote '01 / <year>' into pri-dob AND target-ret — a JANUARY birthday and a January retirement date nobody gave, because the sketch stores an AGE and an age contains no month. MEASURED: {age:44, retire_age:56} -> dob "01 / 1982", ret "01 / 2038". The carry is KEPT and only its route changes: the ages now go straight to slider-age / slider-activation, exactly as this same function already seeds portfolio, contributions and datum — going via an invented Profile date was the only reason a month had to be conjured. ⚠️ The trailing repaint call was gated on `if (_dobEl || _retEl)`, i.e. on the very side effect being removed; it is now unconditional, or the seeded sliders AND the plan-through seed would silently never repaint. (2) _updatePlanEndAge's `|| 43` invented an AGE and it REACHED THE SCREEN: with no DOB and a retirement of 03/2062 the Plan Through slider read 99 yrs against a true 97 for a real DOB of 03/1985 — and the DATE field was already honest (F51 blanks it) WHILE THE SLIDER AND LABEL LIED, the two halves of one quantity disagreeing with only the silent half correct. ⭐ FOUND BY ENUMERATING EVERY WRITE TO THE PROFILE DATE FIELDS RATHER THAN THE THREE PATHS THE PLAN NAMED — the plan named three, there are four. A PLAN'S PATH-LIST IS A NOMINATION. Guarded by scripts/_gate_no_invented_birthday.js, now 7 legs and FOUR controls with SINGLETON red sets, each restoring one shipped fabrication (2026-08-30) // ⛔⛔ FINDING 51, SECOND PASS — THE FIRST ONE DID NOT WORK AND WAS REPORTED AS IF IT HAD. ff4e436 shipped, was smoked on production, and the Captain found the impossible date STILL THERE: typing 08/2055 left "08 / 2148", 08/1900 left "08 / 1993". ⛔ THE DISCRIMINATOR WAS THE TARGET RETIREMENT DATE. _updatePlanEndAge RETURNS EARLY when there is no valid 4-digit retirement year, so the clear (which lives inside _mirrorPlanEnd) was never reached — while the WRITE still happened by a DIFFERENT route: pri-dob's `change` -> _studioApplyProfileDates -> syncFromProfileDates -> _mirrorPlanEnd, from the still-unvalidated text. WRITE ROUTE AND RETRACTION ROUTE WERE NOT THE SAME ROUTE, AND ONLY THE RETRACTION ONE COULD BE SKIPPED. 🔑🔑 AND THE GATE PASSED HONESTLY WHILE PROVING THE WRONG THING: its fixture typed a retirement date "so the derivation would work", and that setup was the ONLY reason the retraction ran. A FIXTURE THAT ESTABLISHES PRECONDITIONS CAN SELECT A DIFFERENT CODE PATH THAN THE USER'S — the same lesson ff4e436's own message recorded, repeated one commit later by the agent who wrote it down. ⭐ FIX: _validatePriDob now calls _mirrorPlanEnd DIRECTLY after _enforceProfileDate, so the retraction is unconditional whatever else is filled in. ⭐ GATE: every refusal now runs TWICE, with and without a retirement date, and the two halves of the fix have SEPARATE controls — --leave-alone reds all four L2 legs, --skip-retraction reds ONLY the no-retirement-date pair, which is exactly the gap the first version could not see (2026-08-30) // ⛔⛔ FINDING 51 — A REFUSED INPUT NO LONGER LEAVES ITS DERIVATIONS BEHIND. Captain-found on production by typing something no smoke step asked for: enter a FUTURE date of birth (03/2057). `pri-dob` fires _updatePlanEndAge on `oninput`, so every keystroke derives through the plan-through mirror UNVALIDATED and wrote 2057+93 = "03 / 2150" into the Profile; `onblur` then refused the age, cleared the DOB, showed the correct message — and LEFT THE IMPOSSIBLE DATE STANDING. Identical at HEAD~1, so pre-existing, not caused by F50. 🔑 A VALUE DERIVED FROM UNVALIDATED INPUT IS ITSELF UNVALIDATED: if the input is later REFUSED, everything derived from it must be retracted in the same breath — "validate on blur, derive on input" guarantees the derivation wins the race, so the retraction has to be explicit or it never happens. THIRD defect of the day whose shape is "two handlers, and the one that runs first wins". ⭐ THE CAUSE WAS ONE WORD: _mirrorPlanEnd's L47 guard `return`ed WITHOUT WRITING when there was no sourced DOB — correct for a COLD field, and exactly wrong for one already holding a value derived from a DOB since refused, where "leave it alone" PRESERVES THE LIE. It now clears. NOT A NEW BEHAVIOUR: _p8_studio_mechanics already asserted "plan-end-age is BLANK with no sourced DOB"; this makes that true BY CONSTRUCTION rather than by what the fixture happened to leave behind. ⚠️ THE REVERT PATH IS UNTOUCHED AND WAS THE GATE'S OWN NEAR-MISS: _enforceProfileDate REVERTS-OR-CLEARS, so with a prior good DOB it restores it and the derived date rightly follows — a first fixture that seeded a good value reported a defect that does not exist. Guarded by scripts/_gate_refused_input_retracted.js, which asserts the PROPERTY (no derived date remains) through TWO rejection shapes, never the literal 2150 (2026-08-30) // ⛔⛔ FINDING 50 — THE PRODUCT NO LONGER INVENTS A DATE THE USER DID NOT GIVE. On a cold, EMPTY profile, touching a Sketch control wrote a BIRTHDAY. MEASURED across three paths, TWO different fabricated months, TWO fields: dragging Current Age wrote pri-dob "06 / 1979" AND target-ret "06 / 2044" (JUNE, from syncFromSketchAges' `: 6` fallback); opening the Current-Age box and pressing Enter WITHOUT TYPING wrote pri-dob "01 / 1986", and the same on Retirement wrote "01 / 2051" (JANUARY, from _dateForField's derived default being committed as though answered). It then fed `current_age` to the engine. 🔑 L47 SOURCED-OR-BLANK WAS ONLY EVER ENFORCED ON THE COPY SURFACE — an unauthored sentence has stopped commits here for months while the code invented dates of birth. It binds the DATA surface identically. ⚠️ A FABRICATED MONTH IS INVISIBLE: plausible, precise, in a field that looks answered, and silently wrong by up to ±11 months on the input the whole model hangs from. ⭐ THE MONTH IS NOW ONLY PRESERVED, NEVER INVENTED — an existing Profile date still keeps its month while the year follows the slider (gate L3 proves the fix did not simply stop writing). ⛔ THE GATE ASSERTS A PROPERTY, NOT A LITERAL: "no `: 6` remains" would have gone GREEN while January was still being invented, since the two paths fabricate different months from different code. Guarded by scripts/_gate_no_invented_birthday.js — two controls, one per path, each restoring the shipped defect (2026-08-30) // §82.766 THE PROFILE'S PLAN-THROUGH DATE ACCEPTS TYPING AGAIN. `change` fires BEFORE `blur`, so syncFromProfileDates' unconditional _mirrorPlanEnd() repainted the field FROM THE SLIDER over what the user had just typed, and the blur-time committer then read the stale text and committed the OLD age — the edit vanished silently, and an invalid date was INDISTINGUISHABLE from a valid one because the mirror handed the validator a good date every time. Captain-confirmed on the live site. ⭐ THE INTERFERER IS REMOVED, NOT THE OWNER: moving the commit into the change listener (the first ruling) would create a SECOND committer whose re-validation of the already-reverted text CLEARS the warning — measured, and rejected on that measurement. ⚠️ The authored warning was never missing, only UNREACHABLE; it renders the correct sentence the moment the path is cleared. Its clear-point is updatePlanThroughLabel — the ONE point every value change passes, which _commitPlanEndDate's failure branch deliberately does not, so a real rejection survives. ⛔⛔ THE REASON THIS SHIPPED AND SURVIVED: _p8_studio_mechanics.js asserted 'invalid date reverts slider + warns' IN GREEN for months by calling _commitPlanEndDate DIRECTLY, never travelling the event path where the defect lived — A GATE THAT INVOKES A HANDLER PROVES THE HANDLER, NOT THE FEATURE. Guarded now by scripts/_gate_plan_through_typed.js, which drives the real UI (click, type, Tab) and never calls the handler (2026-08-30) // §82.717 THE TWO THEME CONTROLS — the Captain's donor toggles ported SHELL YES, BRAIN NO: a site light/dark button beside Upgrade and a canvas-local paper light/dark button, plus 43 donor CSS rules EXTRACTED BY SCRIPT in document order (six `body.light-mode .theme-toggle` blocks across five design revisions; last-wins is PER PROPERTY, so reproducing the ORDER reproduces the result by construction rather than by my arithmetic — hand-resolving that cascade is what certified six live donor blocks as dead on 2026-08-28). ⛔⛔ THIS COMMIT DOES NOT MAKE THE STUDIO LIGHT: it toggles two classes and restyles two buttons, nothing else moves. ⚠️ The parse-time boot block after <body> is the SINGLE DEFINITION of the two storage keys and must stay inline — an external file is a separate request, and a dependency that resolves eventually is not one that resolves at first paint. Guarded by scripts/_gate_theme_toggles.js (2026-08-29) // §82.695 THE VEILS — the chrome now FOLLOWS the paint. Measured 2026-08-29 by flipping all 14 --paint-* to a light palette: seven surfaces were the ENTIRE 'stayed dark' list, every one spelling its colour as a raw rgba(9,18,33,0.9x). A LITERAL CANNOT BE REACHED BY A TOKEN. They now read --veil-92/95/96/97/99, derived from --paint-inkwell in tokens.css's @supports block with byte-exact rgba() fallbacks in :root. ZERO PIXELS MOVED in the dark theme (HEAD vs working tree at a 1/255 threshold; controls: HEAD-vs-HEAD 0.000%, HEAD-vs-white-body 89.077%). ⚠️ FIVE ALPHAS PRESERVED EXACTLY — 0.92/0.95/0.96/0.97/0.99 for one job is probably drift, but MERGING THEM MOVES PIXELS and this commit's whole claim is that it moves none. Guarded by scripts/_gate_veil_reaches.js, whose L3 MOVES THE PAINT rather than pinning a constant — a pinned constant would go green over the very literal this removed (2026-08-29)   // THE STATUS LIGHT'S GLOW FOLLOWS ITS FILL. The 2026-08-28 amendment moved --red onto the estate's negative role and updated the `background` but NOT the `box-shadow` beside it, so the signed-out dot rendered #e24b4a while its halo kept glowing the superseded #e27d61 — two reds on one object, live. 🔑 A RULING WRITTEN AT THE TOKEN LAYER CANNOT REACH A LITERAL SITTING BESIDE IT: `background:var(--red)` moved for free when the token moved; a raw rgba() cannot, because nothing about it participates in a token change. Same alpha, same geometry, hue only. ⚠️ FOUND BY READING THE RECORDED RULING, NOT BY A SWEEP — four proximity sweeps were built and discarded (at dE 15 one could not find this very case; at dE 25 it found it beside four black drop-shadows; filtered further it still flagged --teal beside --teal-mid, which is two real tokens used together). A LITERAL NEAR A ROLE IS NOT A DEFECT SIGNATURE; a fill and its glow being ONE OBJECT is, and that is semantic, not metric. Guarded by scripts/_gate_status_light_glow.js, which asserts the RELATIONSHIP and never a constant (2026-08-29)   // SENTRY WIRED — AND THE CSP IS HALF THE COMMIT. `connect-src` gains https://o4511758659223552.ingest.us.sentry.io, and the head gains two `async` tags: the vendored errors-only SDK (scripts/sentry-10.71.0.min.js) and the canonical init (scripts/datum-sentry.js). ⛔⛔ THE CSP EDIT IS NOT A DETAIL RIDING ALONG — omit it and the SDK loads, initialises, CAPTURES FAITHFULLY, and every envelope dies at the network boundary: AN ERROR MONITOR THAT REPORTS NOTHING LOOKS EXACTLY LIKE AN APPLICATION WITH NO ERRORS, and we would have been MORE blind than before because we would have had a reason to stop looking. FINDING 17; D25 repeating one layer up (Clerk needed the Turnstile allowlist). ⛔ `async`, NEVER `defer`: `defer` executes before DOMContentLoaded so it DELAYS DCL, and this page's preboot rule converts DCL delay 1:1 into BLANK-STAGE TIME — 42187ee's trap, re-answered by _gate_studio_preboot_paint.js. ⭐ SELF-HOSTED so `script-src` is untouched and connect-src is the ONLY policy change; the CDN loader would have needed js.sentry-cdn.com in script-src, re-opening D3's unpinned-dependency hole ON THE ONE COMPONENT WHOSE FAILURE IS INVISIBLE. ⭐⭐ REPLAY IS EXCLUDED BY CONSTRUCTION, NOT BY CONFIG: bundle.min.js (90,590 bytes) ships replayIntegration as a console.warn STUB; bundle.tracing.replay.min.js (267,756) is the one that contains it. A flag can be flipped by a future edit; a bundle that does not contain the code cannot. ⚠️ MEASURED AND IT CHANGED THE DESIGN: 173 gates reference this file and 56 boot it from 127.0.0.1:8001, so init is HOSTNAME-GATED — an unconditional init would have sprayed harness errors into the dashboard on every suite run and buried the real signal. Guarded both directions by scripts/_gate_sentry_wired.js (2026-08-24)   // THE SEED GATE IS SCOPED OFF THE SCRATCH BOOT. `.studio-layout.seed-gated{visibility:hidden}` hides the WHOLE stage while the dossier seed is awaited — correct for a returning user opening real work, and guarding a risk that CANNOT OCCUR on the reload after START FROM SCRATCH, where the user has just asked for founder defaults and `_scratchReset()` has already cleared the prefill sources. WE WERE HIDING THE PAGE TO PROTECT DATA THE USER HAD JUST ASKED US TO THROW AWAY. Measured on production (throttled 40ms/10Mbit/4x CPU, three arms): the reload is EXONERATED — arm B reloads WITHOUT the gate and shows no blank at all; arm C (reload + gate) hid the layout 1158->1611ms = 453ms of exposed blank stage, exposed because ONE FLAG (`datum_auth_hint`) turns the gate ON and, with `datum_studio_overlay_seen`, also removes the overlay that hides it everywhere else. 453ms is a LOWER BOUND — no real session, so the resolver settles early. ⛔ THE FLAG IS LATCHED AT PARSE, NOT READ IN init(): the entry-overlay block CONSUMES it (removeItem) at parse and init() runs at DOMContentLoaded, so the first version of this fix read null every time and changed NOTHING (probe: flagAtStart=1, flagAtGate=null, gate ran anyway). A FLAG WITH A CONSUMER IS A MESSAGE, NOT A STATE. Guarded by scripts/_gate_seed_gate_scratch_skip.js — L2 proves the gate was SCOPED and not deleted, L3 guards the source order whose violation is silent (2026-08-24)   // FINDING 15 — THE DISMISS CONTRACT. The overlay's X ran _startScratchFlow() on FIRST ENTRY and DESTROYED unsaved work: Captain-reproduced on production 2026-08-24 — add a room, reload, the overlay returns, click X, the room is gone. THE RELOAD HAD PRESERVED IT (it sat in datumfi_blueprint_draft_v1); the X deleted it, which is why the obvious story ("reloading an unsaved page reverts it") acquitted the real defect. A CONSENT defect, not a data defect: X means "never mind", START FROM SCRATCH means "discard my work", and both ran the same function. Unsafe BY OMISSION — `_reopened` was added to make RE-ENTRY safe and thereby silently defined first entry as the unprotected path, so nothing in the diff that introduced it looked like a deletion. Compounded by auth: the overlay auto-hides only when `_hintSignedIn && datum_studio_overlay_seen`, so every SIGNED-OUT reload armed it. Scratch remains the only way to discard. Guarded both directions by scripts/_gate_overlay_x_preserves.js, whose --xdead control proves the preserve leg would pass over a dead button (2026-08-24)   // P2 THE DEFER — scripts/studio-account-modal.js becomes `defer` (331,284 bytes, nothing calls it at parse time). HYGIENE, NOT THE ARC'S PAYOFF: measured on the live signed-out page BEFORE the attribute moved, Lighthouse mobile scored 89 / FCP 2.8s / TBT 30ms, and its own render-blocking estimate was 260ms for ALL five head scripts AND the CSS — a 9.3% ceiling against a predicted 15-40%. The prediction was falsified by the baseline. BYTES ARE NOT MILLISECONDS. First paint was never the crisis the split arc was written to solve; that goal is corrected, not dropped (2026-08-23)   // §82.77 ONE BAR — the panel's native scrollbar is DELETED in split and #panel-resizer replaces it: one object that scrolls vertically and resizes horizontally, axis-locked on first intent. The seam LEAVES the scrolling panel for .studio-layout, which is the actual repair — a scrollbar paints above a scroller's own positioned descendants, so a real pointer press at the centre of the divider's own painted hairline moved the panel ZERO px on the shipped bytes. Captain-ruled after asking three times; the previous answer priced "add a drag to the scrollbar" (impossible) instead of "remove the scrollbar" (ordinary) (2026-08-23)   // MOVE 1b — 53 helpers out (132,541 bytes, 1,078 lines, 50 excisions); 1,522,969 -> 1,393,496. One consolidated pointer names all 53, paren-free, inert because every matcher strips comments first (2026-08-22)   // STEP 3 · MOVE 1a — openAccountModal (7981-9555) moved to scripts/studio-account-modal.js; 1,712,678 -> 1,522,969 bytes, 20,333 -> 18,778 lines. The pointer comment names the 76 reads, the two writes, and why they resolve across the boundary (2026-08-22)   // THE SEAM SCOPE FIX + THE DROPDOWN REFLOW. `.studio-layout`'s --studio-panel-w declaration is DELETED — it scoped the token to one subtree while styles/header.css:143 pinned the Sheet·Split·Structure toggle to it from OUTSIDE that subtree, so the toggle sat 80px off the seam at rest and moved 0px when the seam moved 200px (measured, headed Chrome). scripts/studio-panel-resize.js is now the one writer, on documentElement. `.space-dropdown` leaves position:absolute so opening the picker PUSHES the column instead of burying the Next control (2026-08-22)   // §82.15 THE SURFACE PORT: proto2 `.right` ported to .canvas-wrapper as ONE object — 7 layers, 7 background-sizes, both inset shadows, ::before AND ::after. The key light leaves `body` entirely: a global light with surfaces stacked over it was the defect (12% transmission, d=(2,3,3) vs an authored (15,33,24)). D's 76/19 grid lands inside this port because splitting a positional background-size list is the hazard, not the schedule (2026-08-20)
