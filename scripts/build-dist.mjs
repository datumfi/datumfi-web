...
const SACRED = {
  ...
  'studio.html': 'a0d2317566184bb3d13f8afac3317027',// ⛔⛔ FINDING 56 / FINDING 47 — THE WAKE. restoreDraft() had NEVER EXECUTED PAST ITS SECOND LINE, ON ANY LOAD, EVER. Measured with property setters on window: clearDraftAndRefresh assigned t=77ms (inline, parse time), DatumBlueprint t=174ms (defer). The call sat at parse time and its own guard fired every load, 97ms before its dependency existed — the draft was WRITTEN FAITHFULLY AND NEVER READ BACK, so a typed Profile came back EMPTY after a reload. That is F47, found by the Captain reloading a page nobody asked him to reload. FIX: the call moved to init() at DOMContentLoaded, after seedFromBlueprint(), where defer scripts have already run. ⛔ NOT gated on _bpApplied, and that was MEASURED FIRST: on a plain reload DatumBlueprint.load() RETURNS the session draft, so seedFromBlueprint reports success and !_bpApplied would have skipped the restore on exactly the path this fixes — a wake that does nothing, F56 a second time. The exclusion is the explicit ?id=&hydrate=blueprint slot open, so ONE path changes, not two. ⛔ THE BANNER TELLS THE TRUTH TWICE OVER. It announces only if something was ACTUALLY restored — readSessionDraft returns a NON-NULL EMPTY draft on a cold boot carrying the spend field's OWN DEFAULT, which made every cold boot claim a restore over an empty Studio; a value now counts only if it differs from el.defaultValue, the same markup-default test F59 uses. And it WAITS for the entry overlay: the overlay ASKS how to begin while the banner ASSERTS the session is back, and it was measured landing on all three entry choices. The clash is semantic before it is geometric, so sequence fixes it, not position. ⚠️ THE RESTORE IS REACHABLE ONLY VIA THE OVERLAY'S X (#studioCloseIntro): "Start from Scratch" DISCARDS the draft, correctly. Architect flagged that a close button is a strange resume control — his to author, not folded in here. Gate: scripts/_gate_draft_restore_wakes.js (7 legs, REAL path: type -> reload -> the preserving X). The F61 gate's written EXPIRY CONDITION fired and it now travels the real path too — which is the only reason the overlay collision was found. The sweep gate's L1b went RED on cue, forcing F56's exemption out of DECLARED_OPEN in this same commit. (2026-08-31)
