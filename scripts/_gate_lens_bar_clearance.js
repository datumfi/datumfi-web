 ...
 * one figure that says what the home is worth. Moving it to `top:0` clears the drawing by FIVE
 * canvas units against the entry-door swing arcs at y=72.
 *
 * ⛔⛔ FIVE UNITS IS NOT A MARGIN. IT IS SAFE FOR A REASON, AND THE REASON IS WHAT THIS GATE GUARDS.
 ...
 *
 * ⭐⭐ HENCE TWO LEGS, AND THE SECOND IS THE REAL ONE:
 *   LEG 1 asserts the OUTCOME  — clearance > 0 at all eight viewports.
 *   LEG 2 asserts the INVARIANT — clearance IDENTICAL at all eight, which is the coupling itself.
 ...
 *
 * ⛔ NO VIEWPORT IS PRIVILEGED, AND THAT IS DELIBERATE. An earlier report of mine claimed the
 * clearance VARIED and that 1920x1080 was the binding case. That was wrong: one measured point and
 * two bad assumptions (that the bar hung off `.canvas-wrapper`, and that its pixel size stayed fixed
 * while the drawing scaled), extrapolated across eight viewports. Measured properly, they are all
 * equal and NOTHING is binding. Writing that false binding case in here would have been worse than
 * the report — AN ERROR EMBEDDED IN A TEST STOPS LOOKING LIKE AN ERROR AND STARTS LOOKING LIKE A
 * SPECIFICATION, which is exactly how the 62.5 text-stack constant survived.
 *
 ...
 *
 * Usage: node scripts/_gate_lens_bar_clearance.js [LABEL] [--sink] [--unparent]
 *   --sink      pushes the bar down 8px -> it overlaps the entry door. LEG 1 must bite.
...
(async () => {
  ...
  for (const [vw, vh] of VIEWPORTS) {
    ...
    if (SINK) await p.addStyleTag({ content: '.lens-controls { top: 8px !important; }' });
  ...
  console.log('=== ' + LABEL + ' === MODE: ' + (SINK ? 'SINK ' : '') + (UNPARENT ? 'UNPARENT' : '') || 'NORMAL');
  console.log('  viewport      bar bottom   top ink   clearance   offsetParent');
  rows.forEach((r) => console.log('  ' + r.vp.padEnd(12) + String(r.barBottomUnits).padStart(9) +
    String(r.topInk).padStart(10) + String(r.clearance).padStart(12) + '   ' + r.offsetParent));
  ...
  console.log('[_gate_lens_bar_clearance] ' + (fail === 0 ? 'GREEN' : 'RED') + '  ' + pass + '/' + (pass + fail) +
              ((SINK || UNPARENT) ? '   (mutation run: RED IS THE EXPECTED RESULT)' : ''));
  await b.close();
  server.close();
