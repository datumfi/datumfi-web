...
/* AUTO-RESOLVE — the hand-listed callee list above ROTTED the moment studio.html gained
   _yardRentMonthly (2026-08-05, Rule F's named-absent rent seam): this gate died on a ReferenceError
   having asserted nothing at all. A hand-listed callee list is a MAINTAINED DOCUMENT, and documents
   rot. Pull whatever the sandbox reports missing, bounded, and PRINT it — a resolver that silently
   swallowed a name would hide a real extraction failure. Same shape as the newer yard gates.
   ⚠️ IT MUST INVOKE, NOT MERELY COMPILE. The first version only CONSTRUCTED the sandbox, which
   succeeds — _yardRentMonthly is a RUNTIME reference inside _yardIntelligence, so the missing name
   only surfaces when the function is CALLED. A resolver that compiles and stops is testing the wrong
   thing: the same error, one level up, as the one it was written to fix. The loop therefore lives
...
// Resolve by ACTUALLY RENDERING — see the auto-resolve note above; compiling alone proves nothing.
for(let i=0;i<40;i++){
  try{ mk([P,mortLate,helocLate])('p'); break; }
