...
function mutate(b) {
  ...
  const nextRow = o.match(/            <div class="field-row" style="grid-template-columns: 1fr;">\n                <div>\$\{_dLbl\(base, 'Next Payment Date'[\s\S]*?\n            <\/div>\n/);
  ...
  // #3 + #4a — un-swap PMI/Insurance and restore the old insurance label
  swap("'Monthly PMI', 'The lender\\'s safety net'", "'Homeowners / Hazard Insurance (yr)', 'Protecting the asset'");
  swap("'Annual Homeowner Insurance', 'Protecting the asset'", "'Monthly PMI', 'The lender\\'s safety net'");
...
need('#3 escrow order: Property Tax → Monthly PMI', before(M, 'Property Tax (yr)', 'Monthly PMI'));
need('#3 escrow order: Monthly PMI → Annual Homeowner Insurance', before(M, 'Monthly PMI', 'Annual Homeowner Insurance'));
need('#3 escrow order: Annual Homeowner Insurance → Other (yr)', before(M, 'Annual Homeowner Insurance', 'Other (yr)'));
// the swap moved LABELS — the value bindings must not have travelled with them
need('#3 PMI label still bound to pmiMonthly', /Monthly PMI[\s\S]{0,700}?updateAccField\('m1', 'pmiMonthly'/.test(M));
need('#3 insurance label still bound to insAnnual', /Annual Homeowner Insurance[\s\S]{0,700}?updateAccField\('m1', 'insAnnual'/.test(M));
