...
### ② §20 "WHY A 401(k)?" PANEL + §16 BULLETS (2026-07-10, Captain GO) — held-not-committed, gate `_gate_401k_why.js` 16/16 (red-first 1/16)
- **Reusable renderer** `_diWhyPanel(toggleTitle, secList)` extracted (L48) — one panel engine for 401(k) now + the 4 authored siblings later (Rollover 401(k), Taxable, Stock Plan, Crypto — HOLD for Captain GO). Reuses the `.ira-why-*` CSS + expand/collapse toggle from the live 457/IRA panels.
- **`_di401kWhyPanel(acc, base)`** — 14 sections: §20 R404–R413 (one-line answer, Trad-vs-Roth, employer-match [hot], vs-403(b), vs-IRA, Rule-of-55, RMD side, creditor protection, bottom line) + §16 R347–R351 folded in (missed-RMD 25% penalty, small-balance force-out tiers, Roth→Roth-IRA one-way, revenue-sharing, loan-vs-withdrawal). Dispatched on `/401k/` (both rooms), below the decorate button beside the IRA/457 panels.
- **Judgment calls (flagged):** (a) the two comparative sections (R406 Trad-vs-Roth, R411 RMD side) render BOTH [T]/[R] segments — R414 says "gated on taxCode" but the copy references both sides ("split between both", "covers BOTH sides", "not a Roth one"); gating one would break the sentence. (b) inline `[T]`/`[R]`/`/[T]` markers stripped for display. (c) straight apostrophes rendered curly to match the file's panel convention (documented). (d) §16D revenue-sharing placed IN the panel per Captain ("land the §16 bullets inside this panel") though its col-D note suggested a Layer-D append — Captain directive wins.
- No baked limit literals — panel body uses prose "(§15)" references (LOCK-3 display-only). Regression GREEN (401k cert/di/match/reopen, IRA, 457, tax_3a, w7w8, ticker_ui); build GREEN; LF-clean; pin `→ baf14b2c…`.

### ③ §12 `_di401kColTips` (2026-07-10, Captain GO) — held-not-committed, gate `_gate_401k_coltips.js` 16/16 (red-first 1/16)
- **`_di401kColTips(isRoth)`** — 401(k)-aware per-holding column hovers (bank §12 R357–R371: 1a "what it measures" + "why it matters for YOUR retirement" 1b), dispatched in the `_bankCol` merge beside `_di457ColTips`/`_diIraColTips`. Scope = the **12 value columns** (Ticker, Name, Price, Shares Owned, Position Value, Beta, Yield, Geography, Sector, Exp Ratio, Asset Class, Instrument). Plan-menu framing, zero-tax-drag yield, revenue-sharing fee note. [R]/[T] on Beta (R364) + Sector (R367) roth-only bet clauses.
- **⬜ Cost Basis / Unrealized Gain / Acquisition Date (§12 R362/R363/R371)** — deliberately NOT wired as active tips: these render "—" in a 401(k) (§19 R395–R397 canon) via `_diLotColTips` `k401` block, which already carries 401(k)-specific "why it shows —" copy. The §12 versions (which assume the columns are tracked) are superseded by the N/A design.
- **Judgment calls:** unsourced `{planProvider}` (R357) / `{instrumentType}` (R370) parentheticals dropped (L47, same as `_di457ColTips`); curly apostrophes to match sibling convention; keyed by CURRENT COLS labels (`Shares Owned`/`Position Value`).
- Regression GREEN (401k cert/di/match/reopen/why, IRA, 457, tax_3a/11b, w7w8, ticker_ui); build GREEN; LF-clean; pin `→ 80dacff9…`.

**🐛 LATENT BUG FOUND (pre-existing, NOT ③'s doing — FLAG for separate fix):** `_diIraColTips` (4549/4550), `_di457ColTips` (4807/4808), and `_diTaxColTips` (5050/5051) key their Shares/Value entries by `'Shares'` and `'Value'`, but the live COLS labels are `'Shares Owned'` / `'Position Value'` (U2 relabel). Those two tooltips silently stopped matching → IRA/457/taxable Shares-Owned & Position-Value columns fall back to the generic COLS tip. One-line-per-function key rename fixes it (3 rooms) — Captain's call, beyond ③ scope.

### ④ §19 §ROLLUP-PARITY RECONCILE + tooltip-key fix (2026-07-10, Captain per-field ruling) — held, gates `_gate_401k_rollup.js` 16/16 + `_gate_coltip_keys.js` 6/6
Field-by-field diff resolved the two-versions trap without thinning a single live [R]/[T] hover:
- **Already live (byte-identical):** Account Value (R384 = `_diAccountValueCell`) · 3 table cols (R395/396/397 = `_diLotColTips` k401).
- **Group A — §19 INSTALLED** (live 1b was a bare one-liner; §19 richer, no branch loss): Cash % (R387), International (R388, FTC-lost), Weighted Beta (R392), Blended Yield (R393), Avg Expense (R394).
- **Group B — BLEND-APPEND** (Captain ruling: enrich without thinning; §19's unique clause appended to the END of the live [R]/[T] hover, both branches, originals proven intact by gate): **Equity %** (+"no yearly dividend/cap-gains tax… tax-efficiency worry… doesn't apply here"), **Balance** (+"behind the 59½ gate… future retirement income, not liquid cash"; C292 intact), **Unrealized Gain** (+"informational only… no capital-gains math"; B375 intact).
- **Group B — KEEP-LIVE (my readability discretion, FLAGGED):** **Bond %** — §19's "bonds belong here" would read as a *contradiction* right after the live [R] "bonds are slightly wasted in a Roth"; the live [R]/[T] split is more correct (Architect could add §19's "asset-location" naming to the [T] branch only if wanted). **Annual Contribution** — §19's "hard IRS cap" is already stated by the live "same IRS limit / one IRS ceiling"; append would be redundant.
- **🐛 tooltip-key fix (folded into ④'s commit per Captain):** `_diIraColTips`/`_di457ColTips`/`_diTaxColTips` keys `'Shares'→'Shares Owned'`, `'Value'→'Position Value'` (+ matching `t:`), so those two tooltips apply again in IRA/457/taxable. Gate `_gate_coltip_keys.js` red-first 0/6→6/6.
- Full regression GREEN (16 gates incl. 401k cert/di/match/reopen/why/coltips, IRA, 457, tax, w7w8, ticker_ui, room_picker, a5_ownership); build GREEN; LF-clean; pin `→ ce536466…`.

**🏁 401(k) ROOM = COMPLETE** (pending Captain smoke): JOB A reduced surface + ① D144 + copy reconcile (C292/B35/B375) + ② §20/§16 Why-panel + ③ `_di401kColTips` + ④ §19 rollup reconcile. Every authored renderable §19/§20/§16/§12/§9/§8 line is ✅ live, ⬜ deliberate (Bond/Contribution keep-live flagged), or already-wired. Two Architect follow-ups flagged (Bond [T] asset-location naming; the tooltip fix landed). **HELD for Captain smoke of ④ + tooltip → GO → commit → PUSH (per "push held until ④ green").**

---

## 🔌 ROLLOVER 401(k) — "THE CONDUIT" · A3 TRANSIT IDENTITY (2026-07-10, Captain GO all-of-A3) — held-not-committed
Baseline HEAD `ce9c112` (A2 foundation). One held commit. Bank = `Rollover 401(k) Copy Bank` (235 rows). Gates: `_gate_rollover401k_di.js` **21/21** (red-first 8/21 on ce9c112) + NEW `_gate_rollover_countonce.js` **8/8** (red-first: 1/8 on ce9c112 bytes AND load-bearing isolation — neutralize the guard → `buildStudioRequest` payload double-counts $208k, flips RED). Full 35-gate regression: 33 GREEN; the only 2 reds (`_gate_401k_vault` RMD-73 drift, `_gate_stepA_headers` U2 width) are **pre-existing on `ce9c112`** (verified by stash), NOT A3. The 4 Captain-critical gates (401k_cert / 401k_why / room_picker / a5_ownership) all GREEN.

### Line-by-line bank↔live verdict (L50)
- **§0 (R6–R12) instructions / §6 VERIFY (R68–R78) / §10 DOCTRINE / §11 WORKED-EXAMPLE / §0.7 decision-record** — N/A (not renderable). VERIFY honored: V-PRETAX-ROUTE ✅, V-ORIGIN-FIRST ✅ (gated: origin index < strip index), V-EMPLOYER-PERKS ✅, V-CASH-NUDGE ✅, V-DIRECT-WARNING ✅, V-NO-CONTRIB-CEILING ✅, V-REUSE-ENGINE ✅, V-SOURCE-LINE ✅ (in-plan rollover-balance line already live at the Vault, 5527/5966), V-DOCTRINE ✅.
- **§1 SIGNALS (R14–R23)** — ✅ WIRED by REUSE of the `_di401k*` signal engine inside `_diNarrConduit` (L48): S-CASH-DRAG **leads** (Layer C `_diConduitCashLead`, fires ≥15%), S-HIGH-EXPENSE (`_di401kMenuFees`), S-HIGH-BETA/S-BOND-HEAVY (`_di401kBehavior`), S-CONCENTRATED/S-EQUITY-HEAVY (`_di401kTilts` + spine), S-DIV/yield (strip Blended-Yield cell), S-EMPTY (`_DI_EMPTY_STATE`). §1's literal wording is the spec; the render re-voices via Layers A–G per bank §0.4.
- **§2 TITLE HOVERS (R25–R33)** — ✅ WIRED. H-TITLE (R27) is the Conduit title hover (`/rollover/` fork before `/401k/`, so it no longer mis-reads "The Vault"). **Judgment call:** R28–R33 (H-DIRECT-INDIRECT/H-ERISA/H-LOANS/H-RULE55/H-RMD/H-CONVERT) were authored as *chip* hovers, but no discrete chips exist in the modal — their sentences are woven **verbatim** into the title hover's What/Why/Catch so every H-* fact is triggerable on one hover. (Flag: if the Captain wants discrete chips, that's a fast follow.)
- **§3a ORIGIN GATE (R34–R38)** — ✅ WIRED. RO-FROM-401K / RO-DIRECT-DONE / RO-INTO-EMPLOYER-PLAN render as one gold-rule block at the TOP of the DI area, before the DI box + signal strip (V-ORIGIN-FIRST gated). `priorPlan` colors R36 ("an old 403(b)…"), generic "employer plan" fallback when unset (L47).
- **§3b PORTABILITY MODAL (R40–R47)** — ✅ WIRED. All 6 P-* rows (FUNDED-BY-TRANSFER / NO-TAX-ON-ROLL / 60-DAY / ERISA-KEPT / LOAN-KEPT / RULE55-KEPT) REPLACE the flat CONTRIBUTION-LIMITS card for rollover (`if(isRollover){§3b}else{limits}`); real 401k rooms keep the limits UI (cert green). WITHDRAWAL-RULES card (incl. the Rule-55 `useRule55` toggle) intentionally KEPT — Rule-55 is a Conduit perk.
- **§4 CONSTANTS (R49–R58)** — N/A (engineering constants), honored: taxCode `pretax`, no baked RMD/limit literals (rollFlavor='roth' flips only the Layer-E tail).
- **§5 TOGGLES (R60–R66)** — TG-UNIVERSAL ✅ (the live include-in-plan toggle). TG-RULE55 ✅ (`useRule55` renders for rollover + §7C `rule55Eligible` capture). **Judgment calls (content reachable, no discrete toggle):** TG-INVEST-IT → the idle-cash beat is **always-on** in Layer C (per §14 S2 "no new toggle"); TG-LOAN-AWARE → loan education always-on via §2/§3b/Layer D. **⬜ TG-CONVERT-WATCH — DELIBERATELY-BLANK** (needs a low-income-year signal we don't have; the H-CONVERT content is surfaced in the §2 hover instead of a toggle).
- **§7 ASKS (R80–R86)** — ✅ superseded/wired by §7C capture copy.
- **§7.5 LINK + COUNT-ONCE (R88–R96)** — L-DROPDOWN ✅ ("Rolled into…" populated from the user's own IRA/401(k)-family accounts), L-GUARD ✅ (**safe-by-default informational**, in-session opt-in, reload-safe — Captain Fork A), L-DIRECTION ✅ (destination-holds-dollars default), L-SOURCE-OF-TRUTH ✅ (visible "already counted in [X]" flag), L-SYMMETRY ✅ (guard is one `/rollover/` helper `_conduitIsInformational` → reuses for The Passage when built). **⚠️ L-TAXTAIL — PARTIAL:** the rolled dollars' pretax character is preserved (Layer E forks on `rollFlavor`, and the count-once guard keeps them out of any double-count) — but *stamping this room's taxCode onto a sub-line inside the destination's balance-split* is a deeper destination-side feature NOT built (flagged; not silently faked).
- **§9 DI LAYERS A–G (R97–R106)** — Layer A spine (R99) ✅, B tilts (R100) ✅ reuse, B2 composition (R101) ✅ reuse, C behavior + idle-cash lead (R102) ✅, D portability/perks (R103) ✅, E rollFlavor tax fork (R104, pretax default / Roth tail) ✅, G close (R106) ✅. **Layer F (R105) — folded into Layer D** (Rule-55/loan/access overlap heavily; a separate F sentence would be redundant per the insight-layer no-restate rule). Flag: the "otherwise locked until 59½" nuance is carried by the WITHDRAWAL-RULES card, not a separate DI sentence.
- **§12 PER-COLUMN (R126–R135)** — ✅ WIRED. R129 "no contribution-limit comparison — preserved capital" (via §3b); R134 basis tax-irrelevant + the 3 tax-lot cols (see §RP). Equity/value column header hovers **reuse `_di401kColTips`** (401k plan-menu framing) — flag: could be rollover-tuned later; acceptable per bank §12 reuse intent.
- **§13 GEO/SECTOR (R137–R140)** — ✅ WIRED by reuse (`_di401kArchSignals`→`_diTaxSignals` geo/sector, the same maps).
- **§14 SUGGESTIONS (R142–R148)** — S2 idle-cash ✅ (always-on Layer C lead), S3 IRA-vs-401k comparator ✅ (carried by §2/§3b/Layer D), S4 priorPlan enrichment ✅ (§7C capture + §3a color). S1 NUA ⏸️ deferred (no employer-stock-basis input). S5 shared tax-tail ▫ eng-note only.
- **§15 (R150–R157)** — ⬜ DELIBERATELY-NOT-WIRED — SUPERSEDED by §16 (Captain ruling; §15 not wired).
- **§16 WHY-PANEL (R195–R211)** — ✅ WIRED. `_diConduitWhyPanel` on the shared `_diWhyPanel` engine (L48): 8 sections R199–R207 verbatim (one-line answer, consolidation, tax-status-preserved, 4-choices, **Rule-of-55 trap [hot]**, direct-vs-indirect, don't-commingle/backdoor, bottom-line). R208 wiring-note honored (no new-contributions nudge — §RP shows "—"); R209 sources not rendered.
- **§0.6 CTA / NAMING / SHAPE (R158–R164)** — ROOM-NAME "The Conduit" ✅, TAXCODE-WIRING pretax ✅, SHAPE ✅, GATE-LIVE (COUNTIFS) ✅. **⬜ CTA-COPY (R160) NOT-WIRED** — the account-picker descriptive CTA sentence ("Rolled an old 401(k)…") is not surfaced; the picker leaf shows only the name "The Conduit". **Out of A3's modal scope** (a picker-hover feature) — FLAGGED for a Captain call, not silently wired.
- **§RP ROLLUP (R185–R193)** — ✅ Balance consolidation-lens (R188), Annual-Contribution "—" (R189), 3 tax-lot col header hovers (R190–192, new `kRoll` in `_diLotColTips`). The other 8 universal fields inherit the 401(k) baseline via reuse.
- **§7C SUB-FORM (R213–R222)** — ✅ WIRED. 5 fields (priorPlan / rollFlavor / rule55Eligible / planAllowsLoans / linkedToAccount) as a collapsible "Rollover facts" sub-form, all blank-safe, `updateAccField`/`updateAccToggle`→`acc.*`. **⚠️ KNOWN-GAP (Captain Fork A): field + count-once persistence DEFERRED to the D1+KV backend — NO Clerk-metadata bytes spent.** In-session only; the count-once guard defaults SAFE on every load, so a reload can never double-count even with zero persistence.
